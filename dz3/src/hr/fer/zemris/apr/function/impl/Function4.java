...
public class Function4 extends ObjectiveFunction {
    ...
    public Function4() {
        super(new Vector(true, new double[]{0, 0}), new Vector(true, new double[]{3, 0}));
    }
    ...
    public double calculate(Vector x) {
        ...
        resultCount++;
        double result = Math.pow(x.get(0) - 3, 2) + Math.pow(x.get(1), x.get(1));
    ...
    public Vector gradient(Vector x) {
        ...
        return new Vector(new double[] {2 * x.get(0) - 6, 2 * x.get(1)});
    }
